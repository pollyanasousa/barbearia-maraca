...
- **Autenticação (RF02 do PRD é obrigatório em todo o projeto)**: o front-end e o back-end precisam de login/logout protegendo rotas. Isso exige que o modelo de dados suporte credenciais. Decisão de escopo até segunda ordem do time:
  - `barbeiro` possui login (é o administrador único do sistema) — a tabela `barbeiro` deve conter coluna de senha com hash (nunca texto puro).
  - `clientes` **não** possui login nesta versão (agendamento é feito sem conta/senha) — a menos que um card explicitamente mude essa decisão. Se algum card pedir autenticação de cliente, trate como mudança de escopo (ver regra de "Fora de Escopo" abaixo) e sinalize antes de modelar.
  - Biblioteca de hashing e nome exato da coluna (`senha_hash`, `password_hash`, etc.) ficam a critério do primeiro card que tocar autenticação — uma vez definido, siga a regra de "Consistência entre cards" (seção 5) daí em diante.
